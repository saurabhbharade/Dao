	...
	public Company getCompany(int cid) 
	{
		try {
			...
			String sqlQuery = "select * from "+ TABLEcompany + " where "+ COLcid + "="+ "?";
				...
				float salary = rs.getFloat(COLpackage);
				float ssc = rs.getFloat(COLssc);
				float hsc = rs.getFloat(COLhsc);
				float beaggregate = rs.getFloat(COLbeaggregate);
				
				Company company = new Company(cid, backlog, numrequired, poolcampus, cname, designation, location, salary, ssc, hsc, beaggregate);
	...
	public List<Company> getAllCompanies() {
		...
		try
		{
			...
			while(rs.next())
			{
				...
				Company company1 = new Company(cid, backlog, numrequired, poolcampus, cname, designation, location, salary, ssc, hsc, beaggregate);
...
//			pst.setInt(2, c.getBacklog());
//			pst.setInt(3, c.getNumrequired());
//			pst.setInt(4, c.getPoolcampus());
//			pst.setString(5, c.getCname());
//			pst.setString(6, c.getDesignation());
//			pst.setString(7, c.getLocation());
//			pst.setFloat(8, c.getSalary());
//			pst.setFloat(9, c.getSsc());
//			pst.setFloat(10, c.getHsc());
//			pst.setFloat(11, c.getBeaggregate());
	...
	public void updateCompany(Company c) throws CompanyExistsException {
		...
		{
			Connection connection = DBConnection.getConnection();
			String sqlQuery = "update " + TABLEcompany + " set " + COLbacklog + "=?, "+ COLnumrequired + "=?, "+ COLpoolcampus + "=?, "+ COLcname + "=?, "+ COLdesignation + "=?, "+ COLlocation +"=?, "+ COLpackage +"=?, "+ COLssc +"=?, "+ COLhsc +"=?, "+ COLbeaggregate +"=? where cid = " + c.getCid() + ";";
			PreparedStatement pst = connection.prepareStatement(sqlQuery);
